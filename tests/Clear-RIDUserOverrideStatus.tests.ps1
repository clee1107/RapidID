$FunctionName =  "Clear-RIDUserOverrideStatus"
Describe 'Clear-RIDUserOverrideStatus' -Tag 'Clear-RIDUserOverrideStatus','RIDUserOverrideStatus' {
    BeforeAll {
    ...
    Context 'Parameter & Help  Checks' {
		...
		BeforeAll {
			$helpinfo = Get-Help Clear-RIDUserOverrideStatus
		}

		It 'should have Identity as mandatory' {
			{Clear-RIDUserOverrideStatus -Identity $null} | Should -Throw
		}
		It 'should have Help' {
    ...
    Context 'function checks' {
        BeforeAll {
            Mock Set-ADUser {
                [PSCustomObject]@{
                    name                     = 'PesterUser'
                    idautoPersonOverride       = "Some Note(s)"
                }
            }
        }
        It 'should call Set-ADUser' {
            Clear-RIDUserOverrideStatus -Identity "PesterUser"

            Should -Invoke Set-ADUser -Times 1 -Scope It
        }
        It 'should call -clear idautoPersonOverride value' {
            Clear-RIDUserOverrideStatus -Identity "PesterUser"

            Assert-MockCalled 'Set-ADUser' -ParameterFilter { $Clear -eq 'idautoPersonOverride' } -Scope It -Exactly 1
        }

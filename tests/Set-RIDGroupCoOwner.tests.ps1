$FunctionName =  "Set-RIDGroupCoOwner"
Describe 'Set-RIDGroupCoOwner' -Tag 'Set-RIDGroupCoOwner','RIDGroupCoOwner' {
    BeforeAll {
    ...
    Context 'Parameter & Help  Checks' {
		...
		BeforeAll {
			$helpinfo = Get-Help Set-RIDGroupCoOwner
		}

		It 'should have Identity as mandatory' {
			{Set-RIDGroupCoOwner -Identity $null} | Should -Throw
		}
		It 'should have Help' {
    ...
    Context 'function checks' {
        It 'should call Set-ADGroup' {
            Set-RIDGroupCoOwner -Identity "PesterGroup" -Member "Someone"

            Should -Invoke Set-ADGroup -Times 1 -Scope It
        }
        It 'should call -add @(idautogroupcoowners)' {
            Set-RIDGroupCoOwner -Identity "GroupWithLogic" -Member "DN"

            Assert-MockCalled 'Set-ADGroup' -ParameterFilter { $Add.ContainsKey("idautogroupcoowners") } -Scope It -Exactly 1
        }

$FunctionName =  "Remove-RIDGroupCoOwner"
Describe 'Remove-RIDGroupCoOwner' -Tag 'Remove-RIDGroupCoOwner','RIDGroupCoOwner' {
    BeforeAll {
    ...
    Context 'Parameter & Help  Checks' {
		...
		BeforeAll {
			$helpinfo = Get-Help Remove-RIDGroupCoOwner
		}

		It 'should have Identity as mandatory' {
			{Remove-RIDGroupCoOwner -Identity $null} | Should -Throw
		}
		It 'should have Help' {
    ...
    Context 'function checks' {
        It 'should call Set-ADGroup' {
            Remove-RIDGroupCoOwner -Identity "PesterGroup" -Member "Someone"

            Should -Invoke Set-ADGroup -Times 1 -Scope It
        }
        It 'should call -remove @(idautogroupcoowners)' {
            Remove-RIDGroupCoOwner -Identity "GroupWithLogic" -Member "Someone"

            Assert-MockCalled 'Set-ADGroup' -ParameterFilter { $Remove.containskey("idautogroupcoowners") } -Scope It -Exactly 1
        }
    }

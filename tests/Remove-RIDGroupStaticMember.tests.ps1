$FunctionName =  "Remove-RIDGroupStaticMember"
Describe 'Remove-RIDGroupStaticMember' -Tag 'Remove-RIDGroupStaticMember','RIDGroupStaticMember' {
    BeforeAll {
        ...
        Mock Set-ADGroup {
            [PSCustomObject]@{
				name                     = 'PesterGroup'
				idautoGroupStaticIncludes = "Someone"
			}
        }
    }
    Context 'Parameter & Help  Checks' {
		...
		BeforeAll {
			$helpinfo = Get-Help Remove-RIDGroupStaticMember
		}

		It 'should have Identity as mandatory' {
            {Remove-RIDGroupStaticMember -Identity $null} | Should -Throw
        }
        It 'should have Member as mandatory' {
            {Remove-RIDGroupStaticMember -Member $null} | Should -Throw
		}
		It 'should have Help' {
    ...
    Context 'function checks' {
        It 'should call Set-ADGroup' {
            Remove-RIDGroupStaticMember -Identity "PesterGroup" -Member "Someone"

            Should -Invoke Set-ADGroup -Times 1 -Scope It
        }
        It 'should call -remove @(idautoGroupStaticIncludes)' {
            Remove-RIDGroupStaticMember -Identity "GroupWithLogic" -Member "Someone"

            Assert-MockCalled 'Set-ADGroup' -ParameterFilter { $Remove.containskey("idautoGroupStaticIncludes") } -Scope It -Exactly 1
        }
    }
